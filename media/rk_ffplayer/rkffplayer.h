...
public:
    ...
    int64_t         hlsFuzzySeek(int64_t timeUs);
...
private:
	...
	static void 			decode(void* frame, int64_t pts,int64_t dts,DEC_TYPE dec_type,void *me);
	...
	int64_t                 getAacDuration(const char *url);      //add by xhr, for aac duration
	...
	bool    mSeekAfterFirst; //add by xhr, for  no async after seek
	...
	int64_t mDoblyFlag;
	int64_t mSeekAtPrepare;
	int64_t mPreSeek;
	int    mSeekFirstPacket;
	int64_t  mOffsetLength;
	int64_t  mOffsetPosition;
    String8 mUri;
    bool isQvm;
    FILE *fp;
    ...
    int    mFirstFrameDeleteNum;
    int    mSeekByUser;
    int    cpu_fd;
    int    cpuscale_fd;

    int    mDeep;
    InforRecoder* mInforRecoder;
    Vector<int64_t> mSeekTime;
